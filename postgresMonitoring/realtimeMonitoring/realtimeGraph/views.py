...
def get_json_measurement_over_value(request, **kwargs):
    data_result = {}

    measureParam = kwargs.get("measure", None)
    valueR = kwargs.get("value", None)
    stationid = kwargs.get("station", None)
    selectedMeasure = None
    measurements = Measurement.objects.all()
    ...
    stations = Station.objects.filter(id=stationid)
    responseData = Data.objects.filter(
        station__in=stations, measurement__name=selectedMeasure.name, value__gte= valueR, time__gte=start.date(), time__lte=end.date()).values()

    startFormatted = start.strftime("%d/%m/%Y") if start != None else " "
    endFormatted = end.strftime("%d/%m/%Y") if end != None else " "
    
    for data_result in responseData:
        data_result["locations"] = [loc.str() for loc in locations]
        data_result["start"] = startFormatted
        data_result["end"] = endFormatted
        data_result["data"] = data
